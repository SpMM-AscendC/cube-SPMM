...
function main {
    ...
    # 定义输入输出目录
    INPUTS_DIR="/root/autodl-tmp/dataset-reorder/ascendC_dataset/"
    OUTPUT_DIR="../output_re_colcondense_prof"
    MODE="reorder"
    ...
    echo "[$(date +%Y-%m-%d\ %H:%M:%S)] Failed Samples Log - Reorder ColDense" > $FAILURE_LOG
    echo "================================================================" >> $FAILURE_LOG

    # 创建单个样本性能记录文件
    INDIVIDUAL_PERF_LOG="./individual_performance_reorder_coldense2.txt"
    echo "[$(date +%Y-%m-%d\ %H:%M:%S)] Reorder ColDense Individual Sample Performance" > $INDIVIDUAL_PERF_LOG
    ...
    for mtx_file in $(find $INPUTS_DIR -name "*.mtx"); do
        ...
        sample_dir="$category_dir/${sample_name}_re_colcondense2"
        ...
        # 使用msprof进行性能测试
        msprof op --output=./msprof_out_reorder_coldense ./output/execute_spmm_op $m $k $n $window_num $block_num $input_row_ptr $input_col $input_values $input_b $output_c $category $sample_name $MODE $input_ref
        exec_status=$?
        ...
        if [ $exec_status -ne 0 ]; then
            ...
            # 清理可能存在的msprof输出
            if [ -d "./msprof_out_reorder_coldense" ]; then
                rm -rf ./msprof_out_reorder_coldense
            fi
            continue
        fi
        ...
        # 提取性能数据从CSV文件
        perf_value="0"
        op_basic_info_file=$(find ./msprof_out_reorder_coldense -name OpBasicInfo.csv 2>/dev/null | head -n 1)
        if [ -n "$op_basic_info_file" ] && [ -f "$op_basic_info_file" ]; then
            ...
            # 强制删除msprof_out文件夹
            if [ -d "./msprof_out_reorder_coldense" ]; then
                rm -rf ./msprof_out_reorder_coldense && echo "[INFO]: Deleted ./msprof_out_reorder_coldense folder"
            fi
        else
        ...
        echo "[INFO]: Processed sample $sample_name"
    ...
    # 汇总统计并写入result.txt
    echo -e "\n==================== Reorder ColDense Performance Statistics ===================="
    # 追加时间戳，便于追溯
    echo -e "\n[$(date +%Y-%m-%d\ %H:%M:%S)] Reorder ColDense Performance Statistics" >> ./result_reorder_coldense2.txt
    ...
    for cat_dir in "${!perf_sum[@]}"; do
        ...
        # 追加写入result_reorder_coldense.txt
        echo "Category: $cat_name | Directory: $cat_dir | Total Samples: $count | Total Performance: $sum us | Average: $avg us" >> ./result_reorder_coldense.txt
    done
    
    echo "[INFO]: Performance statistics have been written to ./result_reorder_coldense2.txt"

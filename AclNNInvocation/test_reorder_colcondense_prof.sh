...
function main {
    ...
    # 创建单个样本性能记录文件
    INDIVIDUAL_PERF_LOG="../profile_result/individual_result_re_coldense.txt"
    ...
    for mtx_file in $(find $INPUTS_DIR -name "*.mtx"); do
        ...
        # 使用msprof进行性能测试
        msprof op --output=./msprof_out_re_coldense ./output/execute_spmm_op $m $k $n $window_num $block_num $input_row_ptr $input_col $input_values $input_b $output_c $category $sample_name $MODE
        exec_status=$?
        ...
        if [ $exec_status -ne 0 ]; then
            ...
            # # 清理可能存在的msprof输出
            # if [ -d "./msprof_out_re_coldense" ]; then
            #     rm -rf ./msprof_out_re_coldense
            # fi
            continue
        fi
        ...
        # 提取性能数据从CSV文件
        perf_value="0"
        op_basic_info_file=$(find ./msprof_out_re_coldense -name OpBasicInfo.csv 2>/dev/null | head -n 1)
        if [ -n "$op_basic_info_file" ] && [ -f "$op_basic_info_file" ]; then
            ...
            # 强制删除msprof_out文件夹
            if [ -d "./msprof_out_re_coldense" ]; then
                rm -rf ./msprof_out_re_coldense && echo "[INFO]: Deleted ./msprof_out_re_coldense folder"
            fi
        else
    ...
    # 追加时间戳，便于追溯
    echo -e "\n[$(date +%Y-%m-%d\ %H:%M:%S)] ColDense Performance Statistics" >> ../profile_result/result_re_coldense.txt
    ...
    for cat_dir in "${!perf_sum[@]}"; do
        ...
        echo "Category: $cat_name | Directory: $cat_dir | Total Samples: $count | Total Performance: $sum us | Average: $avg us" >> ../profile_result/result_re_coldense.txt
        echo "" >> ../profile_result/result_re_coldense.txt
    done
    echo "" >> $INDIVIDUAL_PERF_LOG

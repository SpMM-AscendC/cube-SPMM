...
function main {
    ...
    # 创建单个样本性能记录文件
    INDIVIDUAL_PERF_LOG="../profile_result/individual_result_coldense.txt"
    ...
    for mtx_file in $(find $INPUTS_DIR -name "*.mtx"); do
        ...
        # 使用msprof进行性能测试
        msprof op --output=./msprof_out_coldense ./output/execute_spmm_op $m $k $n $window_num $block_num $input_row_ptr $input_col $input_values $input_b $output_c $category $sample_name $MODE
        exec_status=$?
    ...
    # 追加时间戳，便于追溯
    echo -e "\n[$(date +%Y-%m-%d\ %H:%M:%S)] ColDense Performance Statistics" >> ../profile_result/result_coldense.txt
    ...
    for cat_dir in "${!perf_sum[@]}"; do
        ...
        echo "Category: $cat_name | Directory: $cat_dir | Total Samples: $count | Total Performance: $sum us | Average: $avg us" >> ../profile_result/result_coldense.txt
        echo "" >> ../profile_result/result_coldense.txt
